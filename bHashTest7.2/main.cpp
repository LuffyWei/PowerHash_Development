...
//    fstream out_1;
//    string out_1_path = "/home/weixun/CLionProjects/bHashTest7.2/data/key_s";
...
//        out_1.open(out_1_path.c_str(), ios::app|ios::ate|ios::binary);
        for(int i=0; i<key_buffer_num; i++){
            ...
            hashBuckets->fill_hash_buckets(countMin, dtostring_main(kBuffer[i].k), kBuffer[i].v_size, frequency_threshold);
//            out_1<<std::fixed<<setprecision(3)<<kBuffer[i].k<<'\t'<<kBuffer[i].v_size<<endl;
        }
        memset(kBuffer, 0, key_buffer_num*sizeof(key_vsize));
//        out_1.close();
//        cout<<"完成一批"<<endl;
...
int main(int argc, char** argv) {
    ...
    string base_path = "/home/weixun/CLionProjects/bHashTest7.2/data";
    //count-min的三个参数
    int count_min_num = 3;
    uint64_t count_min_length = 100000;
    uint64_t bucket_num = 500000;
    //子文件个数
    int sub_file_num = 5;
