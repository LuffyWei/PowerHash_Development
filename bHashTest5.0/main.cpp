...
/**
 * 相对上一个版本，没有采用count-min中的频率作为group大小
 * 而是将每一条数据value的大小精确的记录在key文件中
 * 每一个groupSpill中的小group都以accumulate作为起始写入地址
 *
 * 记录key-vsize文件时，采用整形结构，所以有局限性，只能适用于key是整型的数据集
 */
...
int main(int argc, char** argv) {
    ...
    string base_path = "/home/weixun/CLionProjects/bHashTest5.0/data";
    //count-min的三个参数
    int count_min_num = 3;
