...
int Accumulate::fill_accumulate(string k, int vsize, uint64_t murmur_id) {
...
int Accumulate::transvert_accumulate(uint64_t offset) {
...
int Accumulate::insert_accumulate(string k, string v, uint64_t murmur_id) {
    int accumulate_id = murmur_id % accumulate_num;
...
int Accumulate::group_accumulate(string base_path, char split_kv) {
    ...
    for(int j=0; j<accumulate_num; j++){
        ifstream in;
        in.open((base_path+"/groupSpill").c_str(), ios::in);
        if(!in){
        ...
        fstream out;
        out.open((base_path+"/groupSpill").c_str(), ios::in|ios::out);
