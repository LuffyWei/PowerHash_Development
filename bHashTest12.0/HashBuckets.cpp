...
HashBuckets::HashBuckets(uint64_t biggroup_num,  uint64_t buffer_size_B) {
    ...
    this->big_group_num = 0;
    this->small_total_size = 0;
}
...
int HashBuckets::fill_hash_buckets(CountMin*countMin, string k, int vsize, uint64_t frequency_threshold) {
    const unsigned int seed = 59;
    uint64_t frequency = countMin->get_frequency(k,seed);
    uint64_t murmur_id = MurmurHash64A(k.c_str(),k.size(), seed);
    if(frequency < frequency_threshold){
        small_total_size += (k.size()+1+ vsize+1);
    } else {
        big_kv_num ++;
...
int HashBuckets::grouping(Accumulate*accumulate, string input_path, string base_path, char split_kv, char split_vv) {
    ...
    while (! in.eof()){
        ...
        if(buffer_full_or_not(k, v)){
            flushToGroupSpills(accumulate, base_path, split_kv, split_vv);
            inset(accumulate, base_path, k, v, split_kv);
        } else{
            inset(accumulate, base_path, k, v, split_kv);
        }
        getline(in, line);
...
bool HashBuckets::buffer_full_or_not(string k, string v) {
    if(current_size_B + k.size() + v.size() >= buffer_size_B){
        return true;
    }
    return false;
}

int HashBuckets::inset(Accumulate*accumulate, string base_path, string k, string v, char split_kv) {
    if(bucket_num ==0){ //全是小组
        const unsigned int seed = 59;
        uint64_t murmur_id = MurmurHash64A(k.c_str(), k.size(), seed);
        accumulate->insert_accumulate(k, v, split_kv, murmur_id);
