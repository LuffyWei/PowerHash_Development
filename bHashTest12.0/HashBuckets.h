...
public:
    ...
    int fill_hash_buckets(CountMin*countMin, string k, int vsize, uint64_t frequency_threshold);
    ...
    bool buffer_full_or_not(string k, string v);
    int inset(Accumulate*accumulate, string base_path, string k, string v, char split_kv);
