...
HashBuckets::HashBuckets(uint64_t bucket_num, int sub_file_num, int accumulate_num, uint64_t buffer_size_B) {
...
int HashBuckets::fill_hash_buckets(CountMin*countMin, string k, int vsize, uint64_t frequency_threshold) {
    ...
    if(frequency < frequency_threshold){
        int accumulate_id = (int)(murmur_id % accumulate_num);
...
int HashBuckets::transvert_frequency_to_offset() {
    ...
    for(int i=0; i<sub_file_num; i++){
        ...
        for(int j=0; j<accumulate_num; j++){
            uint64_t current_fk_size = accumulate[i][j].fk_size;
            accumulate[i][j].fk_size = offset;
            offset += current_fk_size;
        }
...
//        out.seekp(accumulate[file_no].fk_size-current_offset, ios::cur);
...
//        }
//        current_offset = accumulate[file_no].fk_size;
