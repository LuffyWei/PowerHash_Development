...
HashBuckets::HashBuckets(uint64_t bigroup_num, int sub_file_num, int KV_buffer_num, uint64_t buffer_size_B) {
    this->bucket_num = bigroup_num;
    this->sub_file_num = sub_file_num;
    this->buffer_size_B = buffer_size_B;
    this->current_size_B = 0;
    this->base = new base_node[bucket_num];
    this->kvBuffer = new KVBuffer(sub_file_num, KV_buffer_num);
...
int HashBuckets::fill_hash_buckets(CountMin*countMin, Accumulate*accumulate, string base_path, string k, string v,  char kv_split,
    ...
    if(frequency < frequency_threshold){
        accumulate->fill_accumulate(k, v, murmur_id, bucket_id);
    } else{
        (base+bucket_id) ->fk_sum += (k.size()+1+v.size()+1);
        /****************insert process*****************/
        //insert first node
        if((base+bucket_id)->next == NULL){
            struct list_node * current = new list_node;
            current->k = k;
            current->fk_size = (k.size()+1+v.size()+1);
            current->next = NULL;
...
//                cout<<ptr->fk_size<<endl;
        ...
        accumulate->transvert_accumulate(i, offset);
    }
...
//                    pt->listOfv.clear();
    ...
    accumulate->flush_accumulate(base_path, file_no, split_kv, split_vv);
    current_size_B = 0;
