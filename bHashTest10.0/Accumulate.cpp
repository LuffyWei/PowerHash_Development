...
int Accumulate::fill_accumulate(string k, string v, uint64_t murmur_id, uint64_t bucket_id) {
    ...
    accumulate[file_no][accumulate_id].fk_size += (k.size()+1+v.size()+1);
...
int Accumulate::group_accumulate(string base_path, char split_kv) {
    for(int i=0; i<sub_file_num; i++){
        ...
        for(int j=0; j<accumulate_num; j++){
            ifstream in;
            in.open((base_path+"/groupSpill/"+inttostring_accu(i)).c_str(), ios::in);
            if(!in){
                cout<<"groupSpill "<<i<<" can not open"<<endl;
                exit(0);
            }
//            cout<<accumulate[i][j].kv_num<<endl;
            in.seekg(accumulate[i][j].fk_size, ios::beg);
...
//            cout<<"装填一组"<<endl;
            ...
            fstream out;
            out.open((base_path+"/groupSpill/"+inttostring_accu(i)).c_str(), ios::in|ios::out);
            out.seekp(accumulate[i][j].fk_size, ios::beg);
