...
public:
    ...
    uint64_t accumulate_size_B;
    uint64_t current_size_B;
    uint64_t accumulate_current_B;

    HashBuckets(uint64_t bucket_num, int sub_file_num, int accumulate_num, uint64_t buffer_size_B, uint64_t accumulate_size_B);
    ...
    bool accumulate_full_or_not(string k, string v);
