...
void group_by(string input_path, string base_path, int count_min_num, uint64_t count_min_length,
              uint64_t bucket_num, int sub_file_num, int accumulate_num, uint64_t buffer_size_B, uint64_t accumulate_size_B,
    ...
    /********************读取key文件，然后填充hash表*********************************/
    HashBuckets * hashBuckets = new HashBuckets(bucket_num, sub_file_num, accumulate_num, buffer_size_B, accumulate_size_B);
...
//    string default_path = "/home/weixun/KV data/twitter";
    ...
    string base_path = "/home/weixun/CLionProjects/bHashTest8.3/data";
    //count-min的三个参数
    int count_min_num = 3;
    ...
    uint64_t buffer_size_B = 3000*1000;
    uint64_t accumulate_size_B = 2000*1000;
    ...
    if(argc < 11){
        ...
        accumulate_size_B = atoi(argv[10])*1000;
        frequency_threshold = atoi(argv[11]);
        accumulate_num = atoi(argv[12]);
        bucket_num_small = atoi(argv[13]);
    }
    ...
    group_by(input_path, base_path, count_min_num, count_min_length, bucket_num, sub_file_num, accumulate_num,
             buffer_size_B, accumulate_size_B, key_buffer_num, KV_buffer_num, frequency_threshold, bucket_num_small);
