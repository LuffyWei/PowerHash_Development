...
void group_by(string input_path, string base_path, int count_min_num, uint64_t count_min_length,
              ...
              int key_buffer_num, int KV_buffer_num, int memory, int alpha){
    Timer timer;
    int getPara(int64_t size_countMin, int64_t size_keyBuffer, int memory, int64_t fixedSize, int64_t offsetSize, int64_t totalGroup, int alpha);
...
//    string default_path = "/home/weixun/KV data/wiki";
    ...
    string base_path = "/home/weixun/CLionProjects/bHashTest9.0/data";
    //count-min的三个参数
    int count_min_num = 3;
    uint64_t count_min_length = 100000;
    uint64_t bucket_num = 43000;
    //子文件个数
    int sub_file_num = 3;
    ...
    cout<<count_min_num << " "<< count_min_length<< " "<< bucket_num<< " "<< sub_file_num<<" "<<key_buffer_num<<" "<<
...
int getPara(int64_t size_countMin, int64_t size_keyBuffer, int memory,
