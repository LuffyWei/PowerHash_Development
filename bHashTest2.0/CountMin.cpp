...
CountMin::CountMin(int count_min_num, uint64_t count_min_length, uint64_t sub_file_num, uint64_t bucket_num,
    ...
    kvBuffer = new KVBuffer(sub_file_num, groupBy_buffer_kvnum);
...
int CountMin::add_key_fillSpill(string base_path, string k, string v, char kv_split) {
    ...
    for(int i=0; i<count_min_num; i++){
        count_min_arrays[i][count_min_hash(murmur_id, i)] += (k.size()+1+v.size()+1);
    }

    /******************写入key文件**************************/
    keyBuffer->inset_to_keyBuffer(base_path, k);
    /*******************写入spill文件******************************/
    int file_id = get_file_id(murmur_id);
    kvBuffer->insertToBuffer(file_id, base_path, k, v, kv_split);
    return 0;
}

int CountMin::get_file_id(uint64_t murmur_id) {
...
int CountMin::count_min_hash(uint64_t murmur_id, int index) {
    int i = (a[index]*murmur_id+b[index])%p%count_min_length;
    return i;
}
