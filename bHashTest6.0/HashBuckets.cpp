...
int HashBuckets::fill_hash_buckets(CountMin*countMin, uint32_t key, uint32_t vsize, uint64_t frequency_threshold) {
    ...
    if(frequency < frequency_threshold){
        int accumulate_id = key % accumulate_num;
...
int HashBuckets::inset(string k, string v) {
    ...
    if((base + bucket_id)->next == NULL){
        ...
        int accumulate_id = atoi(k.c_str()) % accumulate_num;
...
//                    pt->listOfv.clear();
    ...
    for(int i=0; i<accumulate_num; i++){
        if(!accumulate[file_no][i].listOfv.empty()){
            out.seekp(accumulate[file_no][i].fk_size - currest_offset, ios::cur);
            ...
            currest_offset = accumulate[file_no][i].fk_size;
            vector<string> temp;
