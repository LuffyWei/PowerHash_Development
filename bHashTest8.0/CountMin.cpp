...
int CountMin::add_key_fillSpill(string base_path, string k, string v, char kv_split) {
    ...
    for(int i=0; i<count_min_num; i++){
        count_min_arrays[i][count_min_hash(k, i,seed)]++;
    }
...
uint64_t CountMin::get_frequency(string k, const unsigned int seed) {
    ...
    for(int i=0; i<count_min_num; i++){
        if(min_f > count_min_arrays[i][count_min_hash(k, i, seed)]){
            min_f = count_min_arrays[i][count_min_hash(k, i, seed)];
        }
    }
    return min_f;
}
int CountMin::count_min_hash(string k, int index, const unsigned int seed) {
    uint64_t murmur_id = MurmurHash64A(k.c_str(), k.size(), seed);
