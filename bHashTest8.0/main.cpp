...
//        old_accumulate[i] = new uint64_t[accumulate_num];
//    }
//    for(int i=0; i<sub_file_num; i++){
//        for(int j=0; j<accumulate_num; j++){
//            old_accumulate[i][j] = accumulate->accumulate[i][j].fk_size;
//        }
//    }
    /***
    ...
    cout<<"total time 1: "<<timer.elapsed()<<endl;
    delete hashBuckets;
...
//    for(int i=0; i<sub_file_num; i++){
//        for(int j=0; j<accumulate_num; j++){
////            cout<<accumulate->accumulate[i][j].fk_size<<"|||"<<old_accumulate[i][j]<<endl;
//            accumulate->accumulate[i][j].fk_size = old_accumulate[i][j];
//        }
//    }
//    for(int i=0; i<sub_file_num; i++){
//        delete [] old_accumulate[i];
//        old_accumulate[i] = NULL;
//    }
...
//    cout<<"删除完成"<<endl;
    ...
    cout<<"total time 2: "<<timer.elapsed()<<endl;
    delete accumulate;
...
int main(int argc, char** argv) {
    string default_path = "/home/weixun/KV data/pg_message";
//    string default_path = "/home/weixun/KV data/wiki";
    string input_path = "/home/weixun/KV data/";
    string base_path = "/home/weixun/CLionProjects/bHashTest8.0/data";
    //count-min的三个参数
    int count_min_num = 3;
    uint64_t count_min_length = 600000;
    uint64_t bucket_num = 80000;
    //子文件个数
    int sub_file_num = 3;
